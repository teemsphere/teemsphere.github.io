...
| `docker_tag` | auto | Docker tag specifying which `teems` image to use. When unset, `ems_solve()` picks the highest CPU capability level your machine supports (e.g. `x86-64-v3`) that has a matching local image (`teems:x86-64-v3` or `teems:v3`), falling back to `teems:latest`. Set explicitly to pin a tag |
