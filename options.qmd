...
| `docker_tag` | auto | Docker tag specifying which `teems` image to use. When unset, `ems_solve()` picks the highest CPU capability level your machine supports (e.g. `x86-64-v3`, read from `ld.so` inside a local `teems` image, so it works the same on Linux, Windows and macOS) that has a matching local image (`teems:x86-64-v3` or `teems:v3`), falling back to `teems:latest`. Set explicitly to pin a tag |
| `version_check` | `"abort"` | What the solve pre-flight does with the image's answer to `teems-solver -version`, asked once per image per session. The solver's major version must match the package's (the interface contract) and be at least the package's minimum, `1.1.0`, the first solver that answers at all. `"abort"` stops on an old, silent or mismatched image; `"warn"` reports it and runs anyway, for bit-exact reproduction against a pinned old image; `"off"` never asks |
