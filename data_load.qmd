...
## Set mappings
In addition to input files, set mappings for sets that are read in must be provided.
...
v7_data <- ems_data(dat_input = "path/to/flexAgg11c17/gsdfdat.har",
                    ...
                    COMM = "food", # b_t, c_b, cmt, ctl, fsh, gro, mil, mnfcs, oap, ocr, ... 
                    ACTS = "food", # ofd, omt, osd, pcr, pdr, rmk, sgr, svces, v_f, vol, wht
...
External mappings may be provided in the form of a filepath to a two column csv file or data frame equivalent (e.g., tibble data table) where the first column represents origin elements and the second column represents mapped (aggregated) elements.
See [teems-mappings](https://github.com/teemsphere/teems-mappings) for example mappings.
Note that all set elements will be transformed to lowercase within the solver outputs.
...
The `ems_aux()` function prepares auxiliary inputs for injection via the `aux_input` argument.
...
If custom parameter values are desired, these can be loaded in their final format into the [`ems_model()`](https://teemsphere.github.io/model_load.html) function, and no aggregation or modification will take place.
