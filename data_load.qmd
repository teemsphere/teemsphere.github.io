...
This function is also used to input any time steps (for temporally dynamic models) as well as load set mappings for sets that are read into the model (i.e., not constructed from set operations).
...
The default margin sector elements can be viewed and modified using:
...
Internal endowment mappings currently include `labor_agg` (aggregated labor endowments), `labor_diff` (aggregated labor along skilled/unskilled delineartion), and `full`.
...
If you wish to use a classic-based model with newer GTAP databases or vice-versa, `convert_format = TRUE` will convert the underlying database.
