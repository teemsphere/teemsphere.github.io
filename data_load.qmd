...
| ---------------------- | ------ | --------------------------------------------------------------- | -------------------------------------------------------------------- |
| **Param.**             |        |                                                                 |                                                                      |
| $\sigma_{\textsc{d}}$  | ESBD   | Armington CES for domestic/imported allocation                  | $i \in \text{TRAD_COMM}$                                             |
| $\sigma_{\textsc{m}}$  | ESBM   | Armington CES for regional allocation of imports                | $i \in \text{TRAD_COMM}$                                             |
| $\sigma_{\textsc{va}}$ | ESBV   | CES between primary factors in production                       | $j \in \text{ALL_INDS},\ r \in \text{REG}$                           |
| $\gamma$               | INCP   | CDE expansion parameter                                         | $i \in \text{TRAD_COMM},\ r \in \text{REG}$                          |
| $\beta$                | SUBP   | CDE substitution parameter                                      | $i \in \text{TRAD_COMM},\ r \in \text{REG}$                          |
| **Data**               |        |                                                                 |                                                                      |
|                        | EVFA   | Endowments – Firms purchases at agents’ prices                  | $i \in \text{ENDW_COMM},\ j \in \text{PROD_COMM},\ r \in \text{REG}$ |
|                        | VDFA   | Intermediates – Firms' domestic purchases at agents’ prices     | $i \in \text{TRAD_COMM},\ j \in \text{PROD_COMM},\ r \in \text{REG}$ |
|                        | VDGA   | Intermediates – Government domestic purchases at agents’ prices | $i \in \text{TRAD_COMM},\ r \in \text{REG}$                          |
|                        | VDPA   | Intermediates – Household domestic purchases at agents’ prices  | $i \in \text{TRAD_COMM},\ r \in \text{REG}$                          |
|                        | VIFA   | Intermediates – Firms' imports at agents’ prices                | $i \in \text{TRAD_COMM},\ j \in \text{PROD_COMM},\ r \in \text{REG}$ |
|                        | VIGA   | Intermediates – Government imports at agents’ prices            | $i \in \text{TRAD_COMM},\ r \in \text{REG}$                          |
|                        | VIPA   | Intermediates – Household imports at agents’ prices             | $i \in \text{TRAD_COMM},\ r \in \text{REG}$                          |
...
\sigma{\textsc{d}i}
\sum_r
(vdpa{i,r} + vdga_{i,r} + vdfa_{i,r} + vipa_{i,r} + viga_{i,r} + vifa_{i,r})
\right)
}
{
\sum_{i,r}^{i^*}
...
::: {.table-scroll .table-wide}
	...
	Header	Description	Set Index
Param.			
$\sigma_{\textsc{d}}$	ESBD	Armington CES for domestic/imported allocation	$c \in \text{COMM}$
$\sigma_{\textsc{m}}$	ESBM	Armington CES for regional allocation of imports	$c \in \text{COMM}$
$\sigma_{\textsc{va}}$	ESBV	CES between primary factors in production	$a \in \text{ACTS},\ r \in \text{REG}$
...
\begin{equation}
\label{eq:sigmaD_v7}
\hat{\sigma}\!_{\textsc{d}_{c}} = \frac{\sum\limits_{c}\limits^{c^{*}}(\sigma\!_{\textsc{d}_{c}}\sum\limits_{r} (vdpp_{c,r} + vmpp_{c,r} + vdgp_{c,r} + vmgp_{c,r} + vdfp_{c,r} + vmfp_{c,r}))}
                    {\sum\limits_{c,r}\limits^{c^{*}} (vdpp_{c,r} + vmpp_{c,r} + vdgp_{c,r} + vmgp_{c,r} + vdfp_{c,r} + vmfp_{c,r})}
\end{equation}

\begin{equation}
\label{eq:sigmaM_v7}
\hat{\sigma}\!_{\textsc{m}_{c}} = \frac{\sum\limits_{c}\limits^{c^{*}}(\sigma\!_{\textsc{m}_{c}}\sum\limits_{r} (vmpp_{c,r} + vmgp_{c,r} + vmfp_{c,r}))}
      {\sum\limits_{c,r}\limits^{c^{*}} (vmpp_{c,r} + vmgp_{c,r} + vmfp_{c,r})}
\end{equation}

\begin{equation}
\label{eq:sigmaVA_v7}
\hat{\sigma}\!_{\textsc{va}_{a}} = \frac{\sum\limits_{a}\limits^{a^{*}}(\sigma\!_{\textsc{va}_{a}}\sum\limits_{r} (evfp_{a,r}))}
                               {\sum\limits_{a,r}\limits^{a^{*}} (evfp_{a,r})}\\
\end{equation}

\begin{equation}
\label{eq:gamma_v7}
\hat{\gamma}_{c,r} = \frac{\sum\limits_{c,r}\limits^{c^{*}r^{*}}\gamma_{c,r}\sum\limits_{c,r} (vdpp_{c,r} + vmpp_{c,r})}
      {\sum\limits_{c,r}\limits^{c^{*}r^{*}} (vdpp_{c,r} + vmpp_{c,r})}
\end{equation}

\begin{equation}
\label{eq:beta_v7}
\hat{\beta}_{c,r} = \frac{\sum\limits_{c,r}\limits^{c^{*}r^{*}}(\beta_{c,r}\sum\limits_{c,r} (vdpp_{c,r} + vmpp_{c,r}))}
      {\sum\limits_{c,r}\limits^{c^{*}r^{*}} (vdpp_{c,r} + vmpp_{c,r})}
\end{equation}