...
Note that sets in the full form (all elements) and subsetted form (model-specific aggregations) can be loaded using `...` with `ems_model()` and `type = "set"` with [`ems_aux()`](aux_load.qmd).
...
For intertemporal models, `ems_model()` determines temporal dynamics by detecting the timestep header in the Tablo file. By default, the expected header in the GTAP-RE model is `"YEAR"`. If a model uses a different header for timestep data, set it via [`ems_option_set()`](options.qmd):
