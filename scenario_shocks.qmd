...
Grab population data with `REGr` set to "full" to keep regions disaggregated
```r
pop <- ems_data(
...
And simply load as a scenario shock
```r
...
Note that the object "pop" may also be save as a .csv and loaded directly from storage.