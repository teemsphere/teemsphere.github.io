...
## Overview
Simple full variable swaps can be directly inputted as a string into the `swap_in` and `swap_out` arguments of [`ems_deploy()`](deploy.qmd). More complex swaps must be handled with the dedicated `ems_swap()` function.

## Simple (full variable) swaps
Here simple swaps refers swaps in which a variable and all its components are being swapped. This is the most common type of swap and is directly handled within [`ems_deploy()`](deploy.qmd). 
...
```
While the subindices for both variables are the same within GTAPv7.0, note that these may vary in some models such as GTAPv6.2 where the REG subindex is "s" for "qfd" and "r" for "tfd". Both swaps may now be loaded within [`ems_deploy()`](deploy.qmd):
```r
cmf_path <- ems_deploy(
  data = data,
  model = model,
